# Evolution Proposal: Capture the user's backtest-fidelity standard: scalper backtests must use actual recorded option prices with cost_model.py and realistic bid/ask + slippage, reporting net P&L.

- Proposal-ID: evo-2026-08-18-backtest-fidelity-standard
- Status: approved
- Signature: backtest-fidelity-standard
- Created-At: 2026-08-18 19:09
- Last-Seen-At: 2026-08-18 19:09
- Target-File: MEMORY.md
- Trigger-Type: preference
- Confidence: medium

## Why This Matters
- Capture the user's backtest-fidelity standard: scalper backtests must use actual recorded option prices with cost_model.py and realistic bid/ask + slippage, reporting net P&L.
...
- The Nifty option chain must **always show the weekly expiry**: the current week's chain while the weekly expiry is still live, and automatically roll to the next week's chain once the current weekly expiry has passed. (Approved 2026-08-18.)
...
- **Live Alerts section shows every asset**: the Live Alerts section must display buy/sell alerts for ALL assets (Nifty, BTC, Bank Nifty, Sensex) — a BTC (or any asset) alert must appear there, not Nifty-only. (Approved 2026-08-18.)
- **Live Alerts capped at latest 5 + Intraday Signals directly below**: the Live Alerts section keeps only the latest 5 alerts, and the 📊 Intraday Signals card sits directly below the Live Alerts section. (Approved 2026-08-18.)
- **Scrolling price ticker at top**: the dashboard shows a horizontal scrolling ribbon/ticker at the very top displaying the latest price for all assets (Nifty, BTC, Bank Nifty, Sensex). (Approved 2026-08-18.)
...
- **Backtest fidelity standard (approved 2026-08-18)**: the scalper backtest must run on **actual recorded option prices** — option_recorder.py historical bid/ask/LTP/OI/volume/IV/Greeks — not a spot-proxy; every trade must go through **cost_model.py** and the report must show **NET P&L / NET expectancy** with realistic **bid/ask + slippage** applied to execution.
...
## User Approval
- Approve: 批准 evo-2026-08-18-backtest-fidelity-standard
- Reject: 拒绝 evo-2026-08-18-backtest-fidelity-standard
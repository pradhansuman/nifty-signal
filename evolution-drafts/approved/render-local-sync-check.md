# Evolution Proposal: Record the workflow rule from today's local-vs-Render divergence: Render only deploys what is pushed to git, so check git push state first when a feature exists locally but is missing on Render.

- Proposal-ID: evo-2026-08-14-render-local-sync-check
- Status: approved
- Signature: render-local-sync-check
- Created-At: 2026-08-14 14:41
- Last-Seen-At: 2026-08-14 14:41
- Target-File: MEMORY.md
- Trigger-Type: preference
- Confidence: medium

## Why This Matters
- Record the workflow rule from today's local-vs-Render divergence: Render only deploys what is pushed to git, so check git push state first when a feature exists locally but is missing on Render.
...
## User Approval
- Approve: 批准 evo-2026-08-14-render-local-sync-check
- Reject: 拒绝 evo-2026-08-14-render-local-sync-check
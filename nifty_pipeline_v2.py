...
import json, math, sys, os, csv, io, tempfile
...
NSE_PROXY_URL = None  # Google Apps Script web app URL (set via --nse-proxy)
...
def load_option_chain_csv(path):
    ...
    with open(path, "r") as f:
        ...
        for row in reader:
            ...
            if ce_oi or ce_ltp:
                entry["CE"] = {
                    "openInterest": ce_oi, "changeinOpenInterest": ce_oi_chg,
                    "lastPrice": ce_ltp, "totalTradedVolume": ce_vol,
            ...
            if pe_oi or pe_ltp:
                entry["PE"] = {
                    "openInterest": pe_oi, "changeinOpenInterest": pe_oi_chg,
                    "lastPrice": pe_ltp, "totalTradedVolume": pe_vol,
...
def fetch_nse_via_proxy(proxy_url, symbol="NIFTY", expiry_display=None):
    """Fetch NSE option chain via Google Apps Script proxy.
    
    The proxy runs on Google's servers (Indian IP), bypassing NSE's geo-block.
    Returns list of chain entries in the standard format.
    """
    if not expiry_display:
        expiry_display = EXPIRY_DISPLAY
    
    print(f"[PROXY] Fetching via Google Apps Script: {proxy_url}", file=sys.stderr)
    ...
    try:
        ...
        if not data.get("success"):
            print(f"[PROXY] Error: {data.get('error', 'unknown')}", file=sys.stderr)
            return []
        
        # Convert flat CSV rows back to chain format
        chain = []
        for row in data.get("chain", []):
            entry = {"strikePrice": float(row.get("strike", 0))}
            
            ce_oi = float(row.get("ce_oi", 0) or 0)
            pe_oi = float(row.get("pe_oi", 0) or 0)
            
            if ce_oi or float(row.get("ce_ltp", 0) or 0):
                entry["CE"] = {
                    "openInterest": ce_oi,
                    "changeinOpenInterest": float(row.get("ce_oi_chg", 0) or 0),
                    "lastPrice": float(row.get("ce_ltp", 0) or 0),
                    "totalTradedVolume": float(row.get("ce_vol", 0) or 0),
                    "impliedVolatility": float(row.get("ce_iv", 0) or 0),
                }
            if pe_oi or float(row.get("pe_ltp", 0) or 0):
                entry["PE"] = {
                    "openInterest": pe_oi,
                    "changeinOpenInterest": float(row.get("pe_oi_chg", 0) or 0),
                    "lastPrice": float(row.get("pe_ltp", 0) or 0),
                    "totalTradedVolume": float(row.get("pe_vol", 0) or 0),
                    "impliedVolatility": float(row.get("pe_iv", 0) or 0),
                }
            chain.append(entry)
        
        print(f"[PROXY] Received {len(chain)} strikes for {expiry_display}", file=sys.stderr)
        
        # Also save to CSV for caching
        if chain:
            csv_path = os.path.join(tempfile.gettempdir(), f"nse_chain_{expiry_display.replace('-','_')}.csv")
            csv_path = csv_path.replace(" ", "_")
            with open(csv_path, "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=[
                    "strike","ce_ltp","ce_oi","ce_oi_chg","ce_vol","ce_iv",
                    "pe_ltp","pe_oi","pe_oi_chg","pe_vol","pe_iv"
                ])
                writer.writeheader()
                for row in data.get("chain", []):
                    writer.writerow(row)
            print(f"[PROXY] Cached to {csv_path}", file=sys.stderr)
        
        return chain
        
    except requests.exceptions.Timeout:
        print("[PROXY] Request timed out — Apps Script may not be deployed yet", file=sys.stderr)
        return []
...
def main():
    ...
    parser.add_argument("--nse-proxy", default=None, help="Google Apps Script web app URL for NSE data")
    ...
    if args.nse_proxy:
        print(f"  NSE Proxy: {args.nse_proxy}")
    print("="*60); print()
    ...
    # Load option chain: proxy > CSV > none
    chain = []
    if args.nse_proxy:
        chain = fetch_nse_via_proxy(args.nse_proxy, "NIFTY", expiry_display)
    if not chain and chain_csv_path:
        chain = load_option_chain_csv(chain_csv_path)
    
    if not chain and not args.nse_proxy:
        print("[INFO] No option chain source. Supply --chain-csv CSV or --nse-proxy URL.", file=sys.stderr)
        print("[INFO] Steps 3-7 (OI, IV, PCR, Volume) will be empty.", file=sys.stderr)
